import { Observable, Observer, Subject, ReplaySubject, from, of, range, MonoTypeOperatorFunction} from 'rxjs';
...
export class GithubAPIInvoker {
    ...
    constructor(/*logger: Logger,*/ gitHubToken: string) {
        //this.logger = logger
    ...
    getPrsStream(responseData?: Github.Link) {
        if (responseData == null) {
            ...
            // )
            prsObservable.subscribe(responseData => {
                if (responseData.hasOwnProperty("link")) {
                    return this.getPrsStream({"link": responseData.data.link})
                }
            })
            ...
            .pipe(
                catchError(error => {
                    ...
                    return from(this.github.getNextPage(responseData))
                })
            ).pipe()
            prsObservable
                .pipe(debounceTime(10000)) // wait 10 sec before getting next batch of PRs.
                .subscribe(responseData => this.getPrsStream({"link": responseData.data.link}))
            this.subscribePrsListeners(prsObservable)
        }
    }

    getCommitsStreamObservable(pr: Github.AnyResponse)
    {
        return from(
            this.github.pullRequests.getCommits({
                owner: 'magento-engcom',
    ...
    getCommitsStream(prsStream: Observable<any>) {
        ...
        .pipe(mergeMap(responseData => responseData.data.map((commit: any) => commit)))
        this.subscribeCommitsListeners(commitsStream)
    }

    getCommitsDataStreamObservable(sha: String) {
        return from(
            this.github.repos.getCommit({
                owner: 'magento-engcom',
                repo: 'msi', 
    ...
    getCommitsDataStream(commitsStream: Observable<any>) {
    ...
    subscribeCommitsDataListeners(commitsDataStream: Observable<{}>) {
        commitsDataStream.subscribe(this.commitsBodyListeners[0])
    }

    subscribeCommitsListeners(commitsStream: Observable<{}>) {
        commitsStream.subscribe(this.commitsListeners[0])
        this.getCommitsDataStream(commitsStream)
    }

    subscribePrsListeners(prsObservable: Observable<Github.AnyResponse>) {
        const prsStream = prsObservable.pipe(
            mergeMap(responseData => responseData.data.map((pr: any) => pr))
        )
